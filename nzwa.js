...
async function starts() {
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
                                case 'help':
                                        if (!isRegister) return reply(botLangs.daftarB(prefix))
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                if (!iRegister) return reply(botLangs.daftarB(prefix))
                if (isLimit(sender)) return reply(botLangs.limitend(pusname))
				...
				case 'alay':
                    nzwa.updatePresence(from, Presence.composing) 
                    if (!isRsgister) return reply(botLangs.daftarB(prefix))
                    if (isLimit(sender)) return reply(botLangs.limitend(pusname))
                ...
                break
                   ...
                   break
					...
					anu = await fetchJson(`http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`, {method: 'get'})
                                        if (!isUser) return reply(botLangs.daftarB(prefix))
					setTimeout( () => {
